...
class LocusTest(HttpUser):
    ...
    competitions = load_competitions()[0]
    clubs = load_clubs()[0]
    ...
    def post_booking(self):
        self.client.post("/purchase-places", data={"places": 0, "clubs": self.clubs["name"],
                                                   "competition": self.competitions["name"]}, name="purchase-places")

    @task
    def get_board(self):
        self.client.get("/clubs", name="view_club_points")
